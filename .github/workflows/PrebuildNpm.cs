...
namespace Microsoft.Terrapin.Worker.PrebuildLogic
{
    ...
    public static class PrebuildNpm
    {
        ...
        public static async Task RemovePackageLockFalse()
        {
            ...
            const string packageLockFalseRe = @"package-lock\s?=\s?false\r?\n?";
            Regex rx = new Regex(packageLockFalseRe, RegexOptions.Compiled);

            Console.WriteLine($"Checking if {NpmrcFileName} contains '{packageLockFalseRe}'.");
            ...
            Console.WriteLine($"{SectionSeparator} Successfully rewrote {NpmrcFileName} with {packageLockFalse}.");
        }
